...
namespace OdinPlus
{
	public static class Util
	{
		...
		static Sprite LoadCustomTexture(String image)
		{
			var path = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
			var filepath = Path.Combine(path, image);
			var temp = LoadTexture(filepath);
			var cartography = Sprite.Create(temp, new Rect(0, 0, 32, 32), Vector2.zero);
			return cartography;
		}

		static Texture2D LoadTexture(string filepath)
		{
			var flag = cachedTextures.ContainsKey(filepath);
			Texture2D result;
			if(flag)
			{
		...
		static MethodInfo _loadImageMethod;
		public static Texture2D LoadTextureRaw(byte[] file)
		{
			...
			var ok = (bool)_loadImageMethod.Invoke(null, new object[] { texture2D, file });
			if(!ok)
				return null;
			return texture2D;
		}
		...
		public static Sprite LoadResouceIcon(string name) => Util.LoadSpriteFromTexture(Util.LoadTextureRaw(Util.GetResource(Assembly.GetCallingAssembly(), $"OdinPlus.Resources.{name}.png")), 100f);
		...
		static Shader _cachedGhostShader;
		static bool _ghostShaderLogged;
		...
		public static string GetRandomElement(this string[] array) => array[array.Length.RollDices()];
		...
		public static string RemoveClone(this string str) => Regex.Replace(str,"(Clone)","");
		...
		{
			var btype = comp.GetType().BaseType;
			FieldInfo[] finfos = btype.GetFields(bindingFlags);
		...
		{
			FieldInfo[] finfos = comp.GetType().GetFields(bindingFlags);
			foreach(var finfo in finfos)
			{
				finfo.SetValue(comp, finfo.GetValue(other));
				//DBG.blogWarning(finfo+" , "+finfo.GetType() + " , " +finfo.Name);
		...
		const BindingFlags bindingFlags = BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.GetField;
		...
		public static T AddComponentcc<T>(this GameObject go, T toAdd) where T : Component => go.AddComponent(toAdd.GetType()).GetCopyOf(toAdd) as T;
	...
	public static class ZNetExtensions
	{
		...
		public static bool IsLocalInstance(this ZNet znet) => znet.IsServer() && !znet.IsDedicated();

		public static bool IsClientInstance(this ZNet znet) => !znet.IsServer() && !znet.IsDedicated();

		public static bool IsServerInstance(this ZNet znet) => znet.IsServer() && znet.IsDedicated();
