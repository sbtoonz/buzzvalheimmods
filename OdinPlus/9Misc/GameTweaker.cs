...
namespace OdinPlus
{
	...
	{
		internal static List<string> TaskHints = new();
		...
		{
			var tutorialText = new Tutorial.TutorialText
			{
				m_label = "Quest Hints",
		...
		public static string DepakVector2i(Vector2i v2i) => $"{v2i.x}_{v2i.y}";
		public static Vector2i Pak(string str)
		{
			Vector2i val;
			var a = str.Split(new char[] { '_' });
			val.x = int.Parse(a[0]);
			val.y = int.Parse(a[1]);
			return val;
		}
		...
		{
			var array = Physics.OverlapBox(pos, new Vector3(range, range, range));
			if(array == null)
				return false;
		...
		public static string Pak(this Vector2i v2i) => $"{v2i.x}_{v2i.y}";
